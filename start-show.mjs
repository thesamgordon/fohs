...
try {
  ...
  if (portCheck) {
    ...
    console.log(
      `${GRAY}Check if another lobby control terminal window is already open.\n`,
    );

    const lines = portCheck.trim().split("\n");
    const processInfo = (lines[1] || "Unknown Process")
      .replace(/\\x20/g, " ")
      .trim();
    ...
    console.log(
      `${WHITE}${BOLD}To force kill the blocking process, run the following in a new terminal:${RESET}`,
    );
...
try {
  ...
  try {
    ...
    console.error(
      "Failed to resolve both .local and .lan domains: ",
      error.message,
    );
    process.exit(1);
  }
  ...
  for (const folder of foldersToWatch) {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      const files = execSync(
        `find ${fullPath} -type f -newer ${buildPath} 2>/dev/null`,
      ).toString();
...
console.log(
  formatLine(
    ` ${BOLD}${"Display URL:".padEnd(labelWidth)}${RESET} ${RED}${lobbyUrl}${RESET}`,
  ),
);
console.log(
  formatLine(
    ` ${BOLD}${"Remote URL:".padEnd(labelWidth)}${RESET} ${RED}${dashboardUrl}${RESET}`,
  ),
);
console.log(formatLine(""));
console.log(
...
try {
  ...
  console.log(
    `${BOLD}\x1b[31m[!]${RESET} ${GRAY}Could not auto-launch OBS. Please open it manually.${RESET}`,
  );
}
...
export const refreshSource = async () => {
  try {
    ...
    console.error(
      `${RED}✘${RESET} OBS WebSocket Error: ${error.message}. Retrying...`,
    );
    return false;
  }
};
